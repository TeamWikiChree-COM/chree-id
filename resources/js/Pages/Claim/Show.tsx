 ...
 */
export default function ClaimShow({ token, serviceName, email, emailVerified, displayName, hasPassword }: ClaimShowProps) {
    const { externalIdps } = usePage().props;
    const googleAvailable = externalIdps.includes('google');

    const initialMethod: Method = hasPassword || !googleAvailable ? 'password' : 'google';
    ...
    const addPasskey = async (): Promise<void> => {
        ...
        try {
            ...
            setPasskeyError(e instanceof Error ? e.message : 'パスキーを登録できませんでした');
        } finally {
    ...
    return (
        <AuthLayout title="ChreeID を作成" heading="ChreeID を作成">
            ...
            {!hasPassword && (
                <Alert severity="info">元のサービスではパスワードを使っていないようです。同じ方法で引き取れます</Alert>
            )}

            <Tabs value={method} onChange={(_, next: Method) => changeMethod(next)} variant="fullWidth">
            ...
            {method === 'google' ? (
                <Stack spacing={2}>
                    {emailField}
                    <Typography variant="body2" color="text.secondary">
                        このアカウントのメールアドレスと同じ Google アカウントで連携すると、そのまま引き取れます
                    </Typography>
                    <Button
                        component="a"
                    ...
                    <Stack spacing={2}>
                        ...
                        {method === 'passkey' && (
                            <Stack spacing={1}>
                                ...
                                {passkeyRegistered ? (
                                    <Alert severity="success">この端末にパスキーを登録しました</Alert>
                                ) : (
                                    <Button
                                        variant="outlined"
                                        color="inherit"
                        ...
                        <Button
                            type="submit"
                            variant="contained"
                            disabled={processing || (method === 'passkey' && !passkeyRegistered)}
                        >
                            ChreeID を作成する
                        </Button>
                    </Stack>
