...
| 項目 | 選定 | 理由 |
| --- | --- | --- |
| 画面 | Inertia + React + MUI | コントローラから React へ直接データを渡せる。画面のために API を書かずに済み、CORS やトークンの受け渡しも出てこない |
| DB | PostgreSQL | 各サービスとは DB を分けている。サービスとは OIDC とサーバ間 API でしか話さないので、DB を直接見に行く近道を物理的に塞げる |
| ID | ULID | 時系列で並べられ、推測されにくい |

Next.js は採用しない。

- Node の実行環境が要り、本番の共用サーバでは動かない
- 認証基盤でオリジンを分けると、CORS、オリジンをまたぐ Cookie、CSRF の受け渡しが増える。OIDC のリダイレクトも Cookie も、同じオリジンのほうが素直に書ける
- 画面はログイン後の管理画面が中心で、SSR も SEO も要らない
